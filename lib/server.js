...
async function start(config) {
  const BRIDGE_IP = config.get('bridge-ip');
  const BRIDGE_MAC = (config.get('bridge-mac') || '').replace(/[^0-9A-F]/g, '');
  const portName = config.get('serialPort');
